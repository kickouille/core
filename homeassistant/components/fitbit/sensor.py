...
def request_app_setup(hass, config, add_entities, config_path, discovery_info=None):
    ...
    start_url = f"{get_url(hass, allow_internal=False)}{FITBIT_AUTH_CALLBACK_PATH}"
...
def request_oauth_completion(hass):
    ...
    start_url = f"{get_url(hass, allow_internal=False)}{FITBIT_AUTH_START}"
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if None not in (access_token, refresh_token):
        ...
        redirect_uri = (
            f"{get_url(hass, allow_internal=False)}{FITBIT_AUTH_CALLBACK_PATH}"
        )
...
class FitbitAuthCallbackView(HomeAssistantView):
    ...
    def get(self, request):
        ...
        if data.get("code") is not None:
            redirect_uri = f"{get_url(hass, require_current_request=True, allow_internal=False)}{FITBIT_AUTH_CALLBACK_PATH}"
